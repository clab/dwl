...
struct Encoder {
  ...
  explicit Encoder(Model& model) {
    ...
    p_s2h = model.add_parameters({H_DIM, REP_DIM});
    p_hbias = model.add_parameters({H_DIM});
    p_h2t = model.add_parameters({OUTPUT_VOCAB_SIZE, H_DIM});
    p_tbias = model.add_parameters({OUTPUT_VOCAB_SIZE});
  }
...
#define DUMB_ADDITIVE
  ...
  Expression PredictTarget(Expression& s, ComputationGraph& cg) {
    Expression s2h = parameter(cg, p_s2h);
    Expression hbias = parameter(cg, p_hbias);
    Expression h = rectify(affine_transform({hbias, s2h, s}));
    Expression h2t = parameter(cg, p_h2t);
    Expression tbias = parameter(cg, p_tbias);
    return logistic(affine_transform({tbias, h2t, h}));
  }
};
...
int main(int argc, char** argv) {
  ...
  int report = 0;
  unsigned lines = 0;
  vector<float> vtrg(OUTPUT_VOCAB_SIZE, 0.f);
  while(1) {
    ...
    for (unsigned i = 0; i < report_every_i; ++i) {
      if (si == training.size()) {
        ...
        cerr << "**SHUFFLE\n";
        random_shuffle(order.begin(), order.end(), rand0n);
      }
      ...
      Expression t = emb.PredictTarget(s, cg);
      if (si % 1000 == 0) {
        vector<float> v = as_vector(cg.forward());
        for (auto w : sent_pair.second)
          cerr << td.Convert(w) << '\t' << v[w] << endl;
      }

      for (auto w : sent_pair.second)
        vtrg[w] = 1;
      Expression tt = input(cg, {OUTPUT_VOCAB_SIZE}, &vtrg);
      Expression l = binary_log_loss(t, tt);
